...
# ASR model related
tag=
asr_tag=wavlm_libri100_multiview        # Suffix to the result dir for asr model training.
asr_exp=        # Specify the directory path for ASR experiment.
                # If this option is specified, asr_tag is ignored.
asr_stats_dir=exp/asr_stats_wavlm_libri100_multiview  # Specify the directory path for ASR statistics.
...
view=multiView #singleView" or "multiView" for k-means (multiView supports wavlm and hubert)
multilayer_feature=False #whether to use multilayer for km model
...
if [ "${lang}" != noinfo ]; then
    token_listdir=data/${lang}_token_list_${tag}
else
    token_listdir=data/token_list_${tag}
fi
...
if [ ${kmeans_feature} = "mfcc" ]; then  # MFCC has no layer
    ...
    kmeans_feature_conf="{type="s3prl_${view}",conf={s3prl_conf=${s3prl_conf},download_dir=ckpt,multilayer_feature=${multilayer_feature},layer=${layer}}}"
fi
kmeans_feature_dir_name=$(echo "${kmeans_feature}" | tr -d "[" | tr -d "]" | tr "," "+")
km_dir="${expdir}"/kmeans/${kmeans_feature_dir_name}_${nclusters}clusters
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    log "Stage 5a: Perform Kmeans using ${kmeans_feature_dir_name} features"
    ...
    scripts/feats/perform_kmeans_${view}.sh \
    ...
    # Get uniq chars
    if [ ! -f "${km_dir}/../distinct_cjk_token_lists_${tag}" ]; then
        ...
        python3 -c "for i in range(${nclusters}): print(i, chr(int('4e00', 16) + i))" \
            > "${km_dir}/../distinct_cjk_token_lists_${tag}"
    fi
    ...
    if [ "${src_case}" = ts ]; then
        ...
        for dset in "${train_set}" ${train_sp_sets} "${_dev_set}" ${test_sets}; do
            awk '
                ...
                "${data_extract}/${kmeans_feature_dir_name}/${_suf}${dset}/pseudo_labels_km${nclusters}.txt" \
    ...
    for dset in "${train_set}" ${train_sp_sets} "${_dev_set}" ${test_sets}; do
        cp ${data_extract}/${kmeans_feature_dir_name}/${_suf}${dset}/text \
            ${data_extract}/${kmeans_feature_dir_name}/${_suf}${dset}/text.${tgt_case}.${tgt_lang}
    done

    if ${eval_valid_set}; then
        utils/copy_data_dir.sh --validate_opts --non-print ${data_audio}/org/${valid_set} \
            ${data_extract}/${kmeans_feature_dir_name}/${_suf}/${valid_set}
        cp ${data_extract}/${kmeans_feature_dir_name}/${_suf}org/${valid_set}/text.${src_case}.${src_lang} \
            ${data_extract}/${kmeans_feature_dir_name}/${_suf}/${valid_set}
        cp ${data_extract}/${kmeans_feature_dir_name}/${_suf}org/${valid_set}/text.${tgt_case}.${tgt_lang} \
            ${data_extract}/${kmeans_feature_dir_name}/${_suf}/${valid_set}

        utils/fix_data_dir.sh --utt_extra_files "text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
            "${data_extract}/${kmeans_feature_dir_name}/${_suf}/${dset}"
    fi

    if [ -n "${speed_perturb_factors}" ]; then
        _dirs="${data_extract}/${kmeans_feature_dir_name}/${_suf}${dset}/${train_set} "
        for factor in ${speed_perturb_factors}; do
            if python3 -c "assert ${factor} != 1.0" 2>/dev/null; then
                _dirs+="${data_extract}/${kmeans_feature_dir_name}/${_suf}${dset}/${train_set}_sp${factor} "
            fi
        done
        utils/combine_data.sh \
            --extra_files "feats.scp utt2num_frames text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
            "${data_extract}/${kmeans_feature_dir_name}/${_suf}${dset}/${train_set}_sp" ${_dirs}
    fi
fi
    ...
    if [ "${feats_type}" = raw ]; then
        ...
        for dset in ${_dsets}; do
            ...
            for extra_file in ${utt_extra_files}; do
                ...
                for single_file in "${data_extract}/${kmeans_feature_dir_name}/${_suf}${dset}"/*; do
