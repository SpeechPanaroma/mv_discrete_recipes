...
feature_type_dir_name=$(echo "${feature_type}" | tr -d "[" | tr -d "]" | tr "," "+")
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ] && ! [[ " ${skip_stages} " =~ [[:space:]]1[[:space:]] ]]; then
    log "stage 1: Dump ${feature_type} feature"

    if ${use_gpu}; then
        _cmd="${cuda_cmd} --gpu ${ngpu}"
    else
        _cmd="${cpu_cmd}"
    fi
    ...
    for dset in ${_dsets}; do
        echo "Dump SSL ${dset} features to ${featdir}/${feature_type_dir_name}/${suffix}${dset}"
        _dump_dir="${featdir}/${feature_type_dir_name}/${suffix}${dset}"
        _dump_dir=$(echo "$_dump_dir" | sed 's/,/_/g')
        _dump_dir=$(echo "$_dump_dir" | tr -d '[]')
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ] && ! [[ " ${skip_stages} " =~ [[:space:]]2[[:space:]] ]]; then
    ...
    _scp_dir="${featdir}/${feature_type_dir_name}/${suffix}${_dset}"
    _scp_dir=$(echo "$_scp_dir" | sed 's/,/_/g')
    _scp_dir=$(echo "$_scp_dir" | tr -d '[]')

    # select portion of data
    if (( $(echo "${_portion} >= 1.0" | bc -l) )); then
        cp "${_scp_dir}"/feats.scp "${km_dir}/train.scp"
    else
        nutt=$(<"${_scp_dir}"/feats.scp wc -l)
        portion_nutt=$(echo ${nutt} ${_portion} | awk '{print(int($1 * $2)+1)}')

        subset_scp.pl \
            ${portion_nutt} ${_scp_dir}/feats.scp \
    ...
    ${cpu_cmd} --num_threads ${num_threads} ${_logdir}/learn_kmeans.log \
        ${python} pyscripts/utils/learn_multiviewKmeans.py \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    for dset in "${train_set}" "${dev_set}" ${other_sets}; do
        log "Extract labels to ${featdir}/${feature_type_dir_name}/${suffix}${dset}"
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ] && ! [[ " ${skip_stages} " =~ [[:space:]]4[[:space:]] ]]; then
        ...
        if [ -d "${alignment_phoneme_dir}" ]; then
            ...
            ${python} local/measure_teacher_quality.py \
                --lab_dir "${featdir}/${feature_type_dir_name}/${suffix}" \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    ...
    for dset in "${train_set}" "${dev_set}" ${other_sets}; do
        label_dir="${featdir}/${feature_type_dir_name}/${suffix}${dset}"
